...
namespace Api.Application.Controllers
{
    ...
    [ApiController]
    public class ImagensFController : ControllerBase
    {
        public IImagensFService _service { get; set; }
        public ImagensFController(IImagensFService service)
        {
            _service = service;
        }
        ...
        [Authorize("Bearer")]
        [HttpPost]
        public async Task<ActionResult> Post([FromBody] ImagensFDtoCreate dtoCreate)
        {
            if (!ModelState.IsValid)
            {
            ...
            try
            {
                ...
                if (result != null)
                {
                    //return Created(new Uri(Url.Link("GetImagensFWithId", new { id = result.Id })), result);
                    return Ok(result);
                }
                else
                {
        ...
        [Authorize("Bearer")]
        [HttpPut]
        public async Task<ActionResult> Put([FromBody] ImagensFDtoUpdate dtoUpdate)
        {
